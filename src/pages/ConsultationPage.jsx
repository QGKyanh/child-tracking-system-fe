import {
  ...
  const ConsultationPage = () => {
    ...
    const handleOpen = (consultation) => {
      const growthVelocity = consultation?.requestDetails?.children?.[0]?.growthVelocityResult || [];
      ...
      const growthChartData = growthVelocity.map((result) => {
        return {
          ...
          bmi: (
            ((result.weight.weightVelocity ?? 0) / ((result.height.heightVelocity ?? 1) / 100) ** 2)
          ).toFixed(1),
        };
      });
                ...
                <VStack spacing={10} align="stretch">
                  <Box bg="white" p={6} borderRadius="md" boxShadow="md">
                    ...
                    <ResponsiveContainer width="100%" height={300}>
                      <LineChart data={selectedConsultation.requestDetails.children[0].growthChartData}>
                        <CartesianGrid strokeDasharray="3 3" />
                        <XAxis dataKey="age" />
                        <YAxis />
                        <Tooltip />
                        <Legend />
                        <Line type="monotone" dataKey="weight" name="Weight (kg)" stroke="#3182CE" />
                        <Line type="monotone" dataKey="height" name="Height (cm)" stroke="#38A169" />
                        <Line type="monotone" dataKey="bmi" name="BMI" stroke="#ED8936" />
                      </LineChart>
                    </ResponsiveContainer>
                  </Box>
                      ...
                      <table style={{ width: '100%', borderCollapse: 'collapse', borderRadius: '8px', overflow: 'hidden' }}>
                        <thead style={{ backgroundColor: '#f0f4f8' }}>
                          <tr>
                            ...
                            <th style={{ padding: '12px', textAlign: 'center' }}>Weight (kg)</th>
                            <th style={{ padding: '12px', textAlign: 'center' }}>Height (cm)</th>
                            <th style={{ padding: '12px', textAlign: 'center' }}>BMI</th>
                          </tr>
                        </thead>
                        <tbody>
                          {selectedConsultation.requestDetails.children[0].growthChartData.map((row, idx) => (
                            <tr key={idx} style={{ borderBottom: '1px solid #eee' }}>
                              <td style={{ padding: '12px', textAlign: 'center' }}>{row.age}</td>
                              <td style={{ padding: '12px', textAlign: 'center' }}>{row.weight}</td>
                              <td style={{ padding: '12px', textAlign: 'center' }}>{row.height}</td>
                              <td style={{ padding: '12px', textAlign: 'center' }}>{row.bmi}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>

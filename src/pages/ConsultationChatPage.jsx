...
const ConsultationChatPage = () => {
  ...
  const { data: messages = [], isLoading } = useGetMessagesByConsultationIdQuery({ consultationId: consultationId });
  const [createMessage] = useCreateMessageMutation();

  const [text, setText] = useState('');
  const [files, setFiles] = useState([]);
  const [messagesState, setMessages] = useState(messages); // Define setMessages here
  ...
  const handleSendMessage = async () => {
    ...
    // Temporarily add the new message to the messages list
    ...
    try {
      ...
      // Revert the optimistic update if the send fails
